...
from evaluator import evaluate_profiles
from generate_data import generate_messages_per_contact
from profile_store import load_global_profile, load_profile, resolve_profile_contact
from prompt_templates import build_prompt
from style_extractor import MODEL, get_client, load_env_file, load_messages
...
def generate_styled_reply(
    ...
    """Generate one reply using the selected style mode and DeepSeek via OpenRouter."""
...
def generate_style_adapted_response(
    ...
    This is the backend-facing integration point:
    load profiles -> gate confidence -> build prompt -> call LLM -> return metadata.
    """
    ...
    # Load both profiles through the store so missing or malformed JSON falls
    # back to neutral profiles instead of crashing the API.
    ...
    # generate_styled_reply builds the correct prompt template and sends it to
    # the LLM, keeping the lower-level prompt and model code in one place.
